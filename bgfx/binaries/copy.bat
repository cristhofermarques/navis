...
if exist "%windows_amd64_path%" (
    for %%l in (bx bgfx bimg bimg_encode bimg_decode) do (
    ...
    for %%l in (texturec shaderc) do (
        if exist "%windows_amd64_path%\%%lRelease.exe" (
            echo        %%l.exe
            @copy %windows_amd64_path%\%%lRelease.exe %out_path%\%%l.exe
        )
    )
)
