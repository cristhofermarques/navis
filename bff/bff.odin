...
@(private)
unmarshal_recursive :: proc(fields: []Field, id: typeid, parent: rawptr, parent_name: string)
{
    raw_any: runtime.Raw_Any
    raw_any.data = parent
    raw_any.id = id
    ...
    for sf in sfs
    {
        ...
        sf_value := reflect.struct_field_value_by_name(transmute(any)raw_any, sf.name)

        for f in fields do if reflect.is_struct(sf.type)
        {
            raw_sf := transmute(runtime.Raw_Any)sf_value
            unmarshal_recursive(fields, sf.type.id, raw_sf.data, sf_path)
        }
